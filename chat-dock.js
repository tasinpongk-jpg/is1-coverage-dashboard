 ...
 *  - RM picker ("I'm <name>") in localStorage -> personalized suggestion chips,
 *    populated from the dashboard's actual primary RMs (data/tickers.json)
...
(function () {
  ...
  var AGENTS = {
    ...
    pythia: {
      ...
      chips: [
        ...
        "What should I watch in my sectors this week?",
  ...
  // Primary RMs (Thai names), loaded from data/tickers.json at startup.
  var RMS = [];
...
@media(max-width:640px){#is1-dock{right:0;bottom:0;width:100vw;max-width:100vw;height:100dvh;\
 max-height:100dvh;border-radius:0}}";
  ...
  dock.innerHTML =
    ...
    '<label>I\'m <select class="is1d-rm"></select></label></div>' +
  ...
  function populateRms() {
    var saved = localStorage.getItem("is1_rm");
    rmSel.innerHTML = RMS.map(function (r) { return "<option>" + esc(r) + "</option>"; }).join("");
    if (saved && RMS.indexOf(saved) !== -1) rmSel.value = saved;
    renderChips();
  }
  // Load the dashboard's actual primary RMs (and covered tickers) once.
  fetch("data/tickers.json").then(function (r) { return r.json(); }).then(function (d) {
    ...
    populateRms();
  }).catch(function () { state.tickers = state.tickers || new Set(); });
  ...
  function getToken(forcePrompt) {
    ...
    if (!t && forcePrompt) {
      t = (prompt("Enter the coverage chat access token (ask your admin):") || "").trim();
